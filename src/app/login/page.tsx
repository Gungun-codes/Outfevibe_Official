...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Handle unverified email with a clear message + link to signup
      if (
    ...
    <div className="min-h-screen bg-black text-white flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative z-10">
        ...
        >
          ...
          {/* ✅ Unverified email — special callout */}
          {error === "__UNVERIFIED__" && (
            <div className="bg-yellow-500/10 border border-yellow-500/40 px-4 py-4 rounded-lg space-y-2">
              <p className="text-yellow-400 text-sm font-semibold">📧 Email not verified</p>
              <p className="text-yellow-300/70 text-xs leading-relaxed">
                You need to verify your email before logging in. Check your inbox for the 6-digit code we sent when you signed up.
              </p>
              <Link
                href="/signup"
                className="inline-block mt-1 text-xs text-[#d4af7f] hover:underline font-medium"
              >
                Go back to signup to verify →
              </Link>
            </div>
          )}
