...
export function ChipSelector({ options, onSelect, multi = false, actionLabel = "✦ Find My Outfits" }: ChipSelectorProps) {
  ...
  const hasMeta    = options.some((o) => OPTION_META[o] ?? false);
  const hasDesc    = options.some((o) => OPTION_META[o]?.desc ?? false);
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 gap-3 mt-2">
        {options.map((opt) => {
          ...
          return (
            <motion.button key={opt} onClick={() => toggle(opt)} whileTap={committed ? {} : { scale: 0.95 }}
              disabled={!!committed}
  ...
  // ── Platform — ✅ NOW SINGLE SELECT (pick one only) ───────────────────────
  if (isPlatform) {
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-2">
        <div className="flex flex-wrap gap-2">
          {options.map((opt) => {
            ...
            if (committed && !isChosen) return null;
            return (
              <motion.button key={opt} onClick={() => toggle(opt)} whileTap={committed ? {} : { scale: 0.93 }}
                disabled={!!committed}
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 gap-2 mt-2">
        {options.map((opt) => {
          ...
          if (committed && !isChosen) return null;
          return (
            <motion.button key={opt} onClick={() => toggle(opt)} whileTap={committed ? {} : { scale: 0.94 }}
              disabled={!!committed}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap gap-2 mt-2">
      {options.map((opt) => {
        ...
        return (
          <motion.button key={opt} onClick={() => toggle(opt)}
      ...
      {multi && selected.length > 0 && (
        <div className="w-full mt-2">
          <button onClick={() => onSelect(selected.join(","))}
